...
class CustomerFunc{
  public:
    ...
    bool reg_new_device(std::vector<DeviceInfo> vec_device);
    ...
    bool mod_relationship(Relationship modified_relationship);
...
class VenderFunc{
  public:
    ...
    bool reg_new_service(std::vector<ServiceInfo> vec_service);
    std::vector<ServiceInfo> show_service(int v_id);
    bool mod_service(ServiceInfo service);
